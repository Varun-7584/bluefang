...
/// ```
///
class VehCoolantUnitsField extends HookWidget {
  const VehCoolantUnitsField({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<UserPreferencesBloc, UserPreferencesState>(
        buildWhen: (p, c) => p.coolantUOM != c.coolantUOM,
        builder: (context, state) {
          ...
          return BFDropdown(
            ...
            whenChanged: (String? value) {
              language == "es" 
                ? context.read<UserPreferencesBloc>().add(
                  UserPreferencesEvent.coolantUomChanged(
            ...
            dropdownValue: (
              ...
              ReferenceTableDataES.uomFluids[context.read<UserPreferencesBloc>().state.coolantUOM] ?? "S_Liters"
              : (context.read<LanguageBloc>().state.language == "fr"
                ? ReferenceTableDataFR.uomFluids[context.read<UserPreferencesBloc>().state.coolantUOM] ?? "F_Liters"
                : ReferenceTableDataEN.uomFluids[context.read<UserPreferencesBloc>().state.coolantUOM] ?? "Liters"
              )
            ),
          );
        });
