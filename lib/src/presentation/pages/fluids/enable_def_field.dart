...
class EnableDefField extends HookWidget {
  EnableDefField({Key? key}) : super(key: key);
    ...
    return BlocBuilder<UserPreferencesBloc, UserPreferencesState>(
        ...
        builder: (context, state) {
          return BFCheckboxInput(
              isChecked: state.enableDefField,
              label: AppLocalizations.of(context)!.enableDef,
              labelStyle: bodyStyle,
              onThisChanged: (bool newValue) {
                context.read<UserPreferencesBloc>().add(
                    UserPreferencesEvent.enableDefInfo(newValue));
              });
        });
  }
}
