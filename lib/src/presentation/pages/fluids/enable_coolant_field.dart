...
class EnableCoolantField extends HookWidget {
  EnableCoolantField({Key? key}) : super(key: key);
    ...
    return BlocBuilder<UserPreferencesBloc, UserPreferencesState>(
        ...
        builder: (context, state) {
          return BFCheckboxInput(
              isChecked: state.enableCoolantField,
              label: AppLocalizations.of(context)!.enableCoolant,
              labelStyle: bodyStyle,
              onThisChanged: (bool newValue) {
                context.read<UserPreferencesBloc>().add(
                    UserPreferencesEvent.enableCoolantInfo(newValue));
              });
        });
  }
}
