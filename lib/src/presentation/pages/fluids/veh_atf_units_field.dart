...
/// ```
///
class VehATFUnitsField extends HookWidget {
  const VehATFUnitsField({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<UserPreferencesBloc, UserPreferencesState>(
        buildWhen: (p, c) => p.atfUOM != c.atfUOM,
        builder: (context, state) {
          ...
          return BFDropdown(
            ...
            whenChanged: (String? value) {
              language == "es" 
                ? context.read<UserPreferencesBloc>().add(
                  UserPreferencesEvent.atfUomChanged(
            ...
            dropdownValue: (
              ...
              ReferenceTableDataES.uomFluids[context.read<UserPreferencesBloc>().state.atfUOM] ?? "S_Liters"
              : (context.read<LanguageBloc>().state.language == "fr"
                ? ReferenceTableDataFR.uomFluids[context.read<UserPreferencesBloc>().state.atfUOM] ?? "F_Liters"
                : ReferenceTableDataEN.uomFluids[context.read<UserPreferencesBloc>().state.atfUOM] ?? "Liters"
              )
            ),
          );
        });
