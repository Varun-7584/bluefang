...
import 'package:bluefang/src/presentation/pages/fluids/veh_coolant_units_field.dart';
...
class VehCoolantQuantityField extends HookWidget {
  VehCoolantQuantityField({Key? key}) : super(key: key);
        ...
        children: <Widget>[
          Expanded(
            child: BFTextInput(
                ...
                hintText: AppLocalizations.of(context)!.coolantQuantity,
                label: AppLocalizations.of(context)!.coolantQuantity,
                outline: true,
                maxLength: 5,
                inputFormatterList: [
                ...
                onChanged: (String value) {
                  try {
                    if (value == "") {
                      return context
                          .read<FluidsformBloc>()
                          .add(FluidsformEvent.coolantQuantityChanged(0));
                    } else {
                      return context.read<FluidsformBloc>().add(
                          FluidsformEvent.coolantQuantityChanged(
                              double.parse(value)));
                    }
                  } catch (e, s) {
                    if (e is FormatException) {
                      log("Invalid double entered: $value. Storing 0 instead.",
                          name: "CoolantQuantityField");
                ...
                validator: (_) {
                  ...
                  inputFilter.stringMatch(textEditingController.text) == null
                      // TODO: Replace print with something else. Maybe a error? or something else.
                      // ignore: avoid_print
                      ? print("No match.")
                      : inputFilter
